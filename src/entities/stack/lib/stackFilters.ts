import type { Locale, StackDetailItem } from '@/entities/stack'
...
export type StackUiCopy = {
  ...
  sortTokenLabel: string
  categoryTokenLabel: string
  resultsHeading: string
  resultSuffix: string
  emptyMessage: string
}

export const buildStackSortOptions = (
  locale: Locale,
): Array<{ value: StackSortOption; label: string }> =>
  locale === 'ko'
    ? [
...
export const buildStackUiCopy = (locale: Locale): StackUiCopy =>
  locale === 'ko'
    ? {
        ...
        allCategory: '전체',
        resetCta: '필터 초기화',
        filterHeading: 'Stack Controls',
        filterDescription: '',
        sortHint: '',
        categoryHint: '',
        capabilityHeading: '경험 기준',
        capabilityDescription: '',
        activeFiltersLabel: '현재 적용',
        sortTokenLabel: '정렬',
        categoryTokenLabel: '분야',
        resultsHeading: '필터 결과',
        resultSuffix: '개 표시 중',
    ...
    : {
        ...
        filterHeading: 'Stack Controls',
        filterDescription:
          'Set the ordering and category first, then narrow the list with experience-based toggles.',
        sortHint: 'Choose how the matrix should be organized.',
        categoryHint: 'Use the larger select field to focus on one domain quickly.',
        capabilityHeading: 'Experience filters',
        capabilityDescription:
          'Combine the three experience toggles to isolate the most relevant skills.',
        activeFiltersLabel: 'Active now',
        sortTokenLabel: 'Sort',
        categoryTokenLabel: 'Category',
        resultsHeading: 'Filtered results',
