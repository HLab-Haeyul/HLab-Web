...
const renderCodeBlock = (
  ...
  const normalizedLanguage = normalizeFenceLanguage(rawLanguage)
  ...
  const highlighted = highlightCodeBlock(rawCode, normalizedLanguage)

  if (!options.enhanceCodeBlocks) {
    return `<pre${languageAttr}><code${languageClass}>${highlighted}</code></pre>`
  }

  const copyLabel = escapeHtml(options.copyButtonLabel?.trim() || 'Copy')
  const copiedLabel = escapeHtml(options.copiedButtonLabel?.trim() || 'Copied')
  const lineNumbers = (rawCode ? rawCode.split('\n') : [''])
  ...
  return `
    <div class="code-block" data-code-block>
      ...
      <div class="code-block-body">
        ...
        <pre${languageAttr}><code${languageClass}>${highlighted}</code></pre>
      </div>
      <textarea class="code-block-copy-source" aria-hidden="true" tabindex="-1">${escapeHtml(rawCode)}</textarea>
    </div>
  `.trim()
}
...
export const markdownToHtml = (markdown: string, options: MarkdownRenderOptions = {}) => {
  const lines = markdown.replace(/\r\n/g, '\n').split('\n')
  const blocks: string[] = []
